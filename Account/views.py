...

-----END RSA PRIVATE KEY-----
"""
...
def docusign_login():
    # Create JWT token
    jwt_payload = {
        "iss": "c660dadf-b55d-4025-b75e-806a03dd3404",
        "sub": "2c9e7766-c715-4642-851e-8383484aaa49",
    ...
    if response.status_code == 200:
        ...
        print(tokens)
        return tokens
        # return redirect("http://yourapp.com/success")
    else:
...
def create_and_send_envelope():
    ...
    envelope_json = {
        ...
        "recipients": {
            "signers": [
                {
                    "email": "navnit.bhoir@alineahealthcare.in",
                    "name": "navnit bhoir",
                    "recipientId": "12",
                    "routingOrder": "1",
                },
                {
                    "email": "pankaj.sajekar@alineahealthcare.in",
                    "name": "Pankaj Sajekar",
                    "recipientId": "1",
                    "routingOrder": "2",
                }
    ...
    api_url = "https://demo.docusign.net/restapi/v2/accounts/{}/envelopes".format(DOCUSIGN_ACCOUNT_ID)
    token = docusign_login()
    DOCUSIGN_ACCESS_TOKEN = token['access_token']
    headers = {
        "Authorization": "Bearer {}".format(DOCUSIGN_ACCESS_TOKEN),
    ...
    if response.status_code == 201:
        ...
        return envelope_id
        return JsonResponse({"message": "Envelope created and sent successfully", "envelope_id": envelope_id})
    else:
        return JsonResponse({"error": "Failed to create and send envelope"}, status=response.status_code)



def get_envelope_status(envelope_id):
    # Send GET request to retrieve envelope status
    api_url = "https://demo.docusign.net/restapi/v2/accounts/{}/envelopes/{}".format(DOCUSIGN_ACCOUNT_ID, envelope_id)
    headers = {
        "Authorization": "Bearer {}".format(DOCUSIGN_ACCESS_TOKEN),
        "Content-Type": "application/json"
    }
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"envelope_id": envelope_id, "status": envelope_status})
    else:
        return JsonResponse({"error": "Failed to retrieve envelope status"}, status=response.status_code)
